...
def tripleDesEncrypt(sourceData='',tripleDesKey = '11111111111111111111111111111111'):
	...
	if(len(sourceData) <= 0 ):
		logging.error('input data len error')
		return None 
	srceData= create_string_buffer(bytes(sourceData.encode()),128)
	desKey= create_string_buffer(bytes(tripleDesKey.encode()),128)
	result = create_string_buffer(1024)
	memset(result,0,sizeof(result))
	#将数据分成8个字节进行3DES加密，不足8个字节填充0x00补齐8个字节
	ret = libtest.Upple3DesEncrypt64BitsText(desKey,srceData,result )
	if ret < 0:
		return ''
	else:
...
#3DES 解密
def tripleDesDecrypt(sourceData,tripleDesKey = '11111111111111111111111111111111' ):
	ret = 0
	if(len(sourceData) <= 0 ):
		logging.error('input data len error')
		return None 
	...
	memset(result,0,sizeof(result))
	#将数据分成8个字节进行3DES加密，不足8个字节填充0x00补齐8个字节
	ret = libtest.Upple3DesDecrypt64BitsText(desKey,srceData,result )
	if ret < 0:
		return ''
	else:
		return result.value.decode()
...
def DesEncrypt(sourceData='',tripleDesKey = '1111111111111111'):
	...
	memset(result,0,sizeof(result))
	#将数据分成8个字节进行3DES加密，不足8个字节填充0x00补齐8个字节
	ret = libtest.Upple1DesEncrypt64BitsText(desKey,srceData,result )
	if ret < 0:
		return ''
	else:
		return result.value.decode()
...
def DesDecrypt(sourceData='',tripleDesKey = '1111111111111111'):
	...
	memset(result,0,sizeof(result))
	#将数据分成8个字节进行3DES加密，不足8个字节填充0x00补齐8个字节
	ret = libtest.Upple1DesDecrypt64BitsText(desKey,srceData,result )
	if ret < 0:
		return ''
	else:
		return result.value.decode()
...
def GenerateTermMac(macData):
	...
	endtime = time.time()
	logging.info("MACSpend = [%.6f]" % (endtime - start))
...
#logging.info(testMac('' ))
