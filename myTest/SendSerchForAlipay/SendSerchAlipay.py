...
def SerchAllNeedOrderNo():
	...
	if isInterDay == 1:
		...
		sql = "SELECT * FROM t_trans_jnl WHERE  trans_date = DATE('%s') and trans_time BETWEEN TIME('%s') and TIME('%s') AND ((b3_pcode='170000' AND b34_telnumber NOT IN (SELECT b34_telnumber FROM t_trans_jnl WHERE  Channel='3006' AND b39_respcode ='00' AND b3_pcode='380000' ) AND b34_telnumber  IN (SELECT b34_telnumber FROM t_trans_jnl WHERE  Channel='3006' AND b39_respcode='00' AND b3_pcode='170000' )) OR ( b3_pcode='280000' AND b34_telnumber NOT IN (SELECT b34_telnumber FROM t_trans_jnl WHERE  Channel='3006' AND b39_respcode ='00' AND b3_pcode='380000' ) AND b34_telnumber  IN (SELECT b34_telnumber FROM t_trans_jnl WHERE  Channel='3006' AND b39_respcode IN ('','96','Z1') AND b3_pcode='280000' )))" % (twoHourbefor[0:8],twoHourbefor[8:14],oneHourbefor[8:14])
	logging.info (sql)
